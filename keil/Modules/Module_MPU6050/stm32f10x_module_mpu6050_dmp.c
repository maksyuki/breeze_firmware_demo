...
Filename:    stm32f10x_module_mpu6050_dmp.c
Author:      maksyuki
Version:     none
...
u8 MPU6050_DMP_Init(void)
{
    ...
    u8 fifoCount;
    u8 fifoBuffer[128];
    s8 xgOffset, ygOffset, zgOffset;

    // Reset device
    // printf(("\r\n=======配置DMP引擎=========\r\n"));
    // printf(("复位MPU6050...\r\n"));
    MPU6050_Reset();
    Delay_TimeMs(50); // Wait after reset 50ms
    // printf(("禁止休眠模式...\r\n"));
    ...
    // printf(("读取加速度偏移值...\r\n"));
    xgOffset = MPU6050_GetXGyrOffsetTC();
    ygOffset = MPU6050_GetYGyrOffsetTC();
    zgOffset = MPU6050_GetZGyrOffsetTC(); 
    ...
    // printf(("禁止IIC主器件模式...\r\n"));
    MPU6050_SetIICMasterModeEnabled(0);
    // 主控制器的I2C与MPU6050的AUXI2C直通。控制器可以直接访问HMC5883L
    MPU6050_SetIICBypassEnabled(1);
    // printf(("设置从器件地址为0x68...\r\n"));
    MPU6050_SetSlaveAddress(0, 0x68);
    // printf(("复位IIC主器件控制权...\r\n"));
    MPU6050_ResetIICMaster();
    Delay_TimeMs(20);
    ...
    if (MPU6050_WriteProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE, 0, 0, 1))
    {
        ...
        if (MPU6050_WriteProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE))
        {
            ...
            MPU6050_SetXGyrOffsetTC(xgOffset);
            MPU6050_SetYGyrOffsetTC(ygOffset);
            MPU6050_SetZGyrOffsetTC(zgOffset);

            // DMPCalibrate();		//tobe tested

            // printf(("写入最后内存跟新到 1/7 ...\r\n"));
            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = dmpUpdates[pos];
            MPU6050_WriteMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1], 1);
            // printf(("写入最后内存跟新到 2/7 ...\r\n"));
            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = dmpUpdates[pos];
            MPU6050_WriteMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1], 0);
            // printf(("正在复位FIFO...\r\n"));
            MPU6050_ResetFIFO();
            // printf(("正在读取FIFO计数...\r\n"));
...
{
    int i;
    u8* ptr = (u8*) &DMP_DATA; // 准备将FIFO的数据包.
    while ((MPU6050_IsDRY() == 0) && (fifoCount < dmpPacketSize));

    mpuIntStatus = MPU6050_GetIntStatus();
    // Get current FIFO count
	fifoCount = MPU6050_GetFIFOCount();
    // Check for overflow (this should never happen unless our code is too inefficient)
    if ((mpuIntStatus & 0x10) || fifoCount == 1024) // 检测FIFO缓冲区是否爆满溢出
    {
...
void DMP_Covert_Data(void)
{
    ...
    DMP_DATA.dmp_gyroz = ((float)DMP_DATA.GYROz) / 16.4F;
    // acc sensitivity to +/-    4 g
    DMP_DATA.dmp_accx = (((float)DMP_DATA.ACCx) / DMP_ACC_SCALE)*ONE_G;	// 加速度转成单位： m/S^2
